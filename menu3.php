<!-- -----------------MENU ICONOS----------------------------- -->
<div class="container-fluid p-4  fondo-amarillo ">
  <div class="row text-center justify-content-center  fuente-institucional f-x-small">    
    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="#" data-toggle="tooltip" data-placement="auto" title="Ir a " target="_blanck">
                <img class="img-fluid " src="img/iconos/periodico.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Periódico Estudiantil</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="#" data-toggle="tooltip" data-placement="auto" title="Ir a " target="_blanck">
                <img class="img-fluid " src="img/iconos/album.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Albúm Fotográfico</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="#" data-toggle="tooltip" data-placement="auto" title="Ir a " target="_blanck">
                <img class="img-fluid " src="img/iconos/v-institucional.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Video Institucional</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="#" data-toggle="tooltip" data-placement="auto" title="Ir a " target="_blanck">
                <img class="img-fluid " src="img/iconos/astereo.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Aunar Stereo</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="#" data-toggle="tooltip" data-placement="auto" title="Ir a " target="_blanck">
                <img class="img-fluid " src="img/iconos/aunartv.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Aunar TV</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="http://campus.aunar.edu.co/" data-toggle="tooltip" data-placement="auto" title="Ir a Campus Virtual " target="_blanck">
                <img class="img-fluid " src="img/iconos/campus-virtual.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Campus Virtual</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="http://aunartech.aunar.edu.co/" data-toggle="tooltip" data-placement="auto" title="Ir a Aunar Tech" target="_blanck">
                <img class="img-fluid " src="img/iconos/atech.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Aunar Tech</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="http://cursos.aunar.edu.co/" data-toggle="tooltip" data-placement="auto" title="Ir a Campus Presencial" target="_blanck">
                <img class="img-fluid " src="img/iconos/campus-presencial.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Campus Presencial</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="http://docentes.aunar.edu.co/" data-toggle="tooltip" data-placement="auto" title="Ir a Bisel Docentes" target="_blanck">
                <img class="img-fluid " src="img/iconos/bdocentes.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Bisel Docentes</p>
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="http://estudiantes.aunar.edu.co/" data-toggle="tooltip" data-placement="auto" title="Ir a Bisel Estudiantes" target="_blanck">
                <img class="img-fluid " src="img/iconos/bestudiantes.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Bisel Estudiantes</p> 
    </div>


    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="#" data-toggle="tooltip" data-placement="auto" title="Ir a " target="_blanck">
                <img class="img-fluid " src="img/iconos/publicaciones.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Publicaciones</p> </div> 



    <div class=" col-xl-1 col-lg-1 col-md-2  col-sm-2 col-4 ">
        <div class="p-2 ">
            <a href="http://biblioteca.aunar.edu.co/" data-toggle="tooltip" data-placement="auto" title="Ir a Biblíoteca" target="_blanck">
                <img class="img-fluid " src="img/iconos/biblioteca.svg" alt="">
            </a>
        </div>
            <p class="pt-2 mb-0">Biblíoteca</p> </div> 



  </div>
</div>
<!-- -----------------FIN MENÚ ICONOS------------------------- -->